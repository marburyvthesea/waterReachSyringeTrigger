...
%%
trialLength = 60;                           % individual trial length (seconds)
expLengthMins = 20;                          % total length of expn (minutes)
...
% [CHANGE VAR.] 
videoFolder = 'F:\WaterReachData\08082025';         %folder for all mouse trial videos
timestamp = datestr(now, 'mm_dd_yy_HH_MM_SS'); 

mouseID = 'mouse4training_4';                                 %mouse ID
...
% Stop video acquisition
    ...
    disp(['frames saved to ', fileName]);
    close(videoWriter);
    disp(['closed ', fileName]);
